...
def read_book(book):
    ...
    if book == 'PrisonLedger':
        ...
        while received != 'input Selected Exit':
            ...
            'esterInfo| Read about the Grand Maester] \n[Exit| Stop reading]', ['AlchemistInfo', 'Queen\'sServantInfo', 'GrandMaesterInfo', 'Exit'], True)
            if received == 'input Selected AlchemistInfo':
                received = set_dialog('The wine has been sent to the local alchemist for inspection. \n[Next| Next]')
            elif received == 'input Selected Queen\'sServantInfo':
                received = set_dialog('The Queen\'s servant claims she saw the suspect put something in the Queen\'s drink. \n[Next| Next]')
            elif received == 'input Selected GrandMaesterInfo':
                received = set_dialog('The Grand Maester claimed that the currently jailed suspect was falsely accused, but provided no evidence to the guards. \n[Next| Next]')
        action('HideDialog()')
    if book == 'Note_From_King':
        ...
        ' Take this key, escape your cell, and do whatever it takes to uncover the identity of the true murderer. I command it. -King Phillip \n[Next| Next]', ['Next'], True)
    action('HideDialog()')
